...
export class PackageSelection extends React.Component {
    ...
    async importPackage () {
        try {
            const result = await window.mainApi.importPackage();
    ...
    async getPackages () {
        const result = await window.mainApi.listPackages();
        if (result.error) {
            this.setError(result.error.type, result.error.message);
        }
        if (!result.canceled) {
    ...
    async deletePackage (path) {
        const result = await window.mainApi.deletePackage(path);
        if (result.error) {
            this.setError(result.error.type, result.error.message);
        }
        if (!result.canceled) {
    ...
    get packages () {
        const packages = this.state.packages.map(pack => {
            return {
                img: pack.img
            };
        });
